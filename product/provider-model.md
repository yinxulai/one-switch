...
原先的「全量配置导入导出」已删除：配置文档需要同时承载供应商、逻辑模型、调度策略和全局设置，任何一处字段变化都会让旧文件整体失效，而用户真正想搬家的往往只是「另一台机器上的这几个供应商」。供应商是自洽的单元，因此按供应商打包。
...
- 新建的模型会像手工新建那样挂到 `default` 逻辑模型，保证导入后供应商立刻可用；已有模型的调度位置不动，导入不会重排候选队列。
