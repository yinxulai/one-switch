...
const proxyRoot = path.join(root, 'source/server/proxy')
...
function canonicalize(specifier, file) {
  ...
  const serverRoot = `${root.replaceAll(path.sep, '/')}/source/server/`
  const commonRoot = `${root.replaceAll(path.sep, '/')}/source/common/`
  ...
  for (const rule of RULES) {
    ...
    for (const file of files) {
      ...
      for (const raw of readSpecifiers(await readFile(file, 'utf8'))) {
        ...
        if (hit) {
          violations.push(`source/server/proxy/${rule.layer} 层：${relative} → ${specifier}（${hit.why}）`)
        }
      }
    }
