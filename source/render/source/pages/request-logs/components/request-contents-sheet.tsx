...
export function RequestContentsSheet(props: RequestContentsSheetProps) {
  ...
  const stages: RequestStageProps[] = [
    ...
    },
    {
      ...
      sections: [
        { id: sectionKey(converted ? '协议转换后的上游请求' : '发送到真实供应商的请求', `请求头 · ${upstreamLabel}`), label: `请求头 · ${upstreamLabel}`, value: conversion?.upstreamRequestHeaders ?? selectedContent?.requestHeaders ?? null },
        { id: sectionKey(converted ? '协议转换后的上游请求' : '发送到真实供应商的请求', `请求 Body · ${upstreamLabel}`), label: `请求 Body · ${upstreamLabel}`, value: conversion?.requestBody ?? selectedContent?.requestBody ?? null },
      ],
      sectionStates,
      onSectionOpenChange: (id, open) => setSectionStates(current => ({ ...current, [id]: open })),
    },
