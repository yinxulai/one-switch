          ...
          <>
            <Card>
              ...
              <CardContent className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-1.5"><Label htmlFor="listen-host" className="text-xs">监听地址</Label><Input id="listen-host" className="h-8 font-mono text-xs" value={settings.listenHost} onChange={event => setSettings({ ...settings, listenHost: event.target.value })} /><p className="text-[11px] text-muted-foreground">建议保留 127.0.0.1，仅允许本机访问。</p></div>
                <div className="space-y-1.5"><Label htmlFor="listen-port" className="text-xs">监听端口</Label><Input id="listen-port" type="number" min={1} max={65535} className="h-8 text-xs" value={settings.listenPort} onChange={event => setSettings({ ...settings, listenPort: Number(event.target.value) })} /></div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="grid gap-3 pt-0 sm:grid-cols-2 lg:grid-cols-4">
                <div className="space-y-1.5"><Label htmlFor="failure-threshold" className="text-xs">连续失败阈值</Label><Input id="failure-threshold" type="number" min={1} className="h-8 text-xs" value={settings.consecutiveFailureThreshold} onChange={event => setSettings({ ...settings, consecutiveFailureThreshold: Number(event.target.value) })} /></div>
                ...
                <div className="space-y-1.5"><Label htmlFor="idle-timeout" className="text-xs">流式空闲超时（毫秒）</Label><Input id="idle-timeout" type="number" min={1} className="h-8 text-xs" value={settings.idleTimeoutMilliseconds} onChange={event => setSettings({ ...settings, idleTimeoutMilliseconds: Number(event.target.value) })} /></div>
              </CardContent>
            </Card>
