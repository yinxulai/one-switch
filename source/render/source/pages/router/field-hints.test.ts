...
describe('resolveInputHints', () => {
  it('仅提供真实上游字段并沿多级连接传递', () => {
    ...
    expect(hints.fields.map(field => field.path)).toEqual(expect.arrayContaining(['request.body.model', 'metadata.controls.mode', 'metadata.protocol']))
    expect(hints.fields.find(field => field.path === 'metadata.controls.mode')).toMatchObject({ valueType: 'enum', enumOptions: ['fast', 'safe'] })
  })
  it('按协议分支收窄枚举并忽略孤立节点', () => {
    const target = condition('target')
  ...
  it('声明队列选择节点的顶层队列字段', () => {
    const target = condition('target')
    const queueSelect: WorkflowNodeModel = { id: 'queue-select', kind: 'queue-select', name: '队列选择', enabled: true, description: '', position, queueIds: ['model-a', 'model-b'] }
