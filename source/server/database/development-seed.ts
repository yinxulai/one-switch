...
export async function seedDevelopmentData(secretStore: KeychainApi, options: DevelopmentSeedOptions = {}): Promise<boolean> {
  ...
  db.transaction(transaction => {
    ...
    transaction.insert(requestContents).values(sampleRequests.flatMap(request => {
      ...
      const requestBody = JSON.stringify({ model: request.provider.name, messages: [{ role: 'user', content: request.index % 3 === 0 ? '请总结这段开发环境示例内容。' : '请生成一段开发环境示例回复。' }], temperature: request.index % 2 === 0 ? 0.7 : 0.2, stream: request.index % 4 === 0 })
    ...
    transaction.insert(attemptContents).values(sampleRequests.filter(request => request.index % 4 === 0).map(request => ({
      ...
      requestBody: JSON.stringify({ model: request.provider.name, messages: [{ role: 'user', content: '这是上游 attempt 级请求正文。' }], stream: true }),
      responseStatus: request.failed ? 504 : 200,
      responseHeaders: JSON.stringify({ 'content-type': 'application/json', 'x-upstream-request-id': `upstream-${request.id}` }),
      responseBody: JSON.stringify({ id: `attempt-${request.id}`, object: 'chat.completion', choices: [{ index: 0, message: { role: 'assistant', content: '这是 attempt 级响应。' } }] }),
      createdTime: request.createdTime,
      updatedTime: request.createdTime,
    }))).run()
  })
