...
const TestSchema = z.object({ rule: ModificationRuleSchema, testCase: z.object({ stage: z.enum(['request', 'response']), body: z.string().max(2 * 1024 * 1024), headers: z.string().max(64 * 1024), clientProtocol: z.string(), upstreamProtocol: z.string(), logicalModelId: z.string(), providerModelId: z.string(), path: z.string(), streaming: z.boolean() }) })
export const modificationRuleRoutes: Record<string, ManagementHandler> = {
  ...
  '/api/modification-rule/test': async (_req, res, body) => {
