...
describe('传输实现解析', () => {
  it('returns the implementation matching the requested kind', () => {
    expect(resolveTransport({ kind: 'http', resolveIdleTimeoutMilliseconds: () => 0 }).kind).toBe('http')
  })
  ...
  it('fails loudly for a declared-but-unimplemented transport instead of falling back to http', () => {
    ...
    expect(() => resolveTransport({ kind: 'websocket' })).toThrow(/WebSocket 传输尚未实现/)
  })
  ...
  it('refuses an http transport without an idle timeout source instead of silently never timing out', () => {
    ...
    expect(() => resolveTransport({ kind: 'http' })).toThrow(/resolveIdleTimeoutMilliseconds/)
  })
})
