...
function collectReferencedKeys(): ScanResult {
  ...
  const files = [
    ...collectSourceFiles(path.join(SOURCE_ROOT, 'render')),
    ...collectSourceFiles(path.join(SOURCE_ROOT, 'command')),
    ...collectSourceFiles(path.join(SOURCE_ROOT, 'common')),
    ...collectSourceFiles(path.join(SOURCE_ROOT, 'server')),
  ]
