...
**零协议转换。** 代理不解析、不修改、不转换任何协议的报文结构。每个上游模型端点直接配置某协议下的完整上游地址；代理根据请求 path 自动识别协议类型，只在配置了该协议端点的上游模型中选择。
...
| [data-model.md](./data-model.md) | 完整数据模型：数据库表设计、Zod Schema、索引、关系 |
