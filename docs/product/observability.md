...
- **格子正方形、列数由格数推出**（块宽高比 6:1，`resolveHeatColumns`），单格上限 26px。格数只由范围决定，同一范围内不会随时间变，块的形状因此是稳的。
...
- 位置固定在第一行右侧，与供应商分布共用 `lg:grid-cols-[1fr_2fr]`：看「谁在扛量」和看「什么时候在扛量」是同一眼的两个问题。两种画法共用这一张卡与这一个标题，换画法不动布局（卡片高度会差十几像素，那是柱状图要给坐标轴留位置）。
