...
- 不代理 Electron 自动更新、GitHub 发布页、[匿名使用统计](./telemetry.md)或其他非模型网络流量：统计上报用独立直连连接器，用户配的代理只服务于模型请求；
- 不在应用内编辑或托管 PAC 脚本；
- 不为正在进行的请求动态切换代理；
...
已有覆盖：`packages/core/source/infrastructure/network/outbound-proxy.test.ts`（URL 归一化与协议白名单、凭据脱敏、bypass 匹配、Chromium 规则转换、模式与 URL 校验）与 `core-network.test.ts`（直连链路的请求 / 响应，代理链路错误包装）。以下为完整目标清单，未列出的部分尚未覆盖：
...
**尚未编写**：当前 `core-network.test.ts` 只在直连链路上启本地目标服务器，没有起本地代理服务器或 `CONNECT` 隧道。以下为目标清单——测试中启动本地目标服务器和本地代理服务器，不依赖公网：
...
**尚未编写**：`packages/console/source/pages/runtime-settings/` 目前没有任何测试文件。以下为人工验收清单：
...
以下条目需要在真实代理环境（含需要认证的代理与 PAC 环境）下人工逐条回归；未完成前不计入发布验收。标注「单测已覆盖」表示其纯逻辑部分已由单测验证，端到端行为仍需人工回归。

- [ ] 用户可以明确选择不使用任何代理、使用系统代理或自定义代理
- [ ] 新安装及旧配置缺少代理模式字段时默认使用系统代理（`SettingsSchema.outboundProxyMode` 默认 `system`）
- [ ] 不使用任何代理时强制直连，不读取系统代理或代理环境变量
- [ ] 系统代理模式遵守操作系统静态代理、PAC 和 DIRECT 解析结果（单测已覆盖 `PROXY` / `SOCKS5` / `DIRECT` 规则转换与未知规则报错）
- [ ] 自定义模式支持 HTTP、HTTPS、SOCKS4 和 SOCKS5 代理地址（单测已覆盖 `normalizeProxyUrl` 的协议白名单）
- [ ] 自定义代理 URL 允许携带用户名和密码并可完成代理认证（单测只覆盖凭据原样保留，代理认证本身未验证）
- [ ] 不合法 URL、未知协议和非法端口无法保存或测试（单测已覆盖 `validateOutboundProxyModeAndUrl`）
- [ ] 默认绕过 localhost、127.0.0.1 和 ::1，本地模型访问不受影响（默认值 `localhost,127.0.0.1,::1`，单测已覆盖精确主机 / 域名后缀 / 端口 / 大小写 / IPv4 / IPv6）
...
- [ ] 运行日志、请求日志、错误消息和测试结果不出现代理认证凭据（单测已覆盖 `redactProxyUrl` 不输出用户名 / 密码 / 查询参数）
- [x] `pnpm typecheck`、`pnpm lint` 和完整测试通过（已验证：`pnpm typecheck` ✓、`pnpm lint` ✓（含包边界 272 文件）、`pnpm test:server` 118 文件 / 1316 例 ✓）
