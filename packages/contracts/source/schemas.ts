...
export const SettingsSchema = z.object({
  ...
  /**
   * 匿名使用统计的开关。**默认关闭**——统计是「用户选择参与」，不是「用户选择退出」。
   *
   * 关闭时不上报任何请求，包括关闭这个动作本身之后的一切后续事件；
   * 关闭的那一刻会尽力补发一条 `telemetry_toggled`，否则服务端只能看到「某天起不再出现」，
   * 分不清是关闭、卸载还是断网（见 `docs/product/telemetry.md` §13）。
   *
