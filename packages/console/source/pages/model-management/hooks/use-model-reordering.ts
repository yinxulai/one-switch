...
export function useModelReordering(selectedModels: ProviderModelRoute[], setModels: Dispatch<SetStateAction<ProviderModelRoute[]>>, reload: () => Promise<void>) {
  ...
  const mutation = useMutation({ mutationFn: (updates: PriorityUpdate[]) => Promise.all(updates.map(update => unwrap(providerModelApi.update(update.id, { logicalModelId: 'default', priority: update.priority })))), onError: async error => { toast.error(t('models.reorder.failed', { message: error.message })); await reload() } })
