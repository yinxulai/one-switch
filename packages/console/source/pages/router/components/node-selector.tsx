...
type NodeSelectorProps = {
  ...
  className?: string
  /** 节点已选中时端口按钮常驻（对应上游的 `data.selected && 'opacity-100'`） */
...
/**
 * 节点选择器。
 * 结构复制自上游 `app/components/workflow/block-selector/index.tsx`，
  ...
  const defaultTrigger = (
    <button
      ...
      className={cn(
        ...
        // 逐字对应上游 block-selector 触发器的 `opacity-0 pointer-events-none group-hover:opacity-100`：
        // 默认收起，悬浮节点或已选中时才显形。
      ...
      // 上游的 block-selector 同样只在 click 上 `stopPropagation()`。
    >
  ...
  return (
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent
        ...
        align="center"
        sideOffset={10}
        // `workflow-ui-surface`：浮层被 portal 到 body，脱离画布作用域，需要自带圆角还原标记。
        // `gap-0 rounded-xl p-1.5 shadow-none ring-0` 覆盖 shadcn PopoverContent 默认的圆角 / 阴影 / 描边。
        className={cn('workflow-ui-surface w-64 gap-0 overflow-hidden rounded-xl border-[0.5px] border-components-panel-border bg-components-panel-bg p-1.5 shadow-none ring-0', className)}
