...
/** 工作流面板按钮的变体名，只保留本仓库用到的几个。 */
export type WorkflowButtonVariant = 'primary' | 'secondary' | 'ghost' | 'ghost-destructive'
/** 工作流面板按钮的尺寸，`small` 用于面板内的次级操作。 */
export type WorkflowButtonSize = 'small' | 'medium'

const SIZE_CLASSNAME: Record<WorkflowButtonSize, string> = {
  small: 'h-6 gap-1 rounded-md px-2.25 text-xs font-medium',
  medium: 'h-8 gap-1 rounded-lg px-3.5 text-[13px] leading-4 font-medium',
}

const VARIANT_CLASSNAME: Record<WorkflowButtonVariant, string> = {
...
type WorkflowButtonProps = ButtonHTMLAttributes<HTMLButtonElement> & {
  ...
  size?: WorkflowButtonSize
}

/**
 * 工作流画布与面板统一使用的按钮。
 * 类名逐字复制自上游 UI 包的 `button/index.tsx` 的 `buttonVariants`，
 ...
 */
export const WorkflowButton = forwardRef<HTMLButtonElement, WorkflowButtonProps>(function WorkflowButton(params, ref) {
