...
// 命令行宿主构建。两步，顺序不能换：
//
//   1. `vite build` → `dist/index.js`（`bin` 入口）与按需加载的分包
...
const distDirectory = path.join(cliDirectory, 'dist')
const consoleDistDirectory = path.resolve(cliDirectory, '../../packages/console/dist')
...
const main = async () => {
  ...
  fs.cpSync(consoleDistDirectory, webDirectory, { recursive: true })
